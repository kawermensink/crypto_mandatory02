...
server_public_key_hex = client_socket.recv(64).decode('utf-8')  # Assuming 64 is the length of the public key
print(f'Received server_public_key_hex: {server_public_key_hex}')
server_public_key = nacl.public.PublicKey(server_public_key_hex, encoder=nacl.encoding.HexEncoder)
# client_box = Box(skclient, server_public_key)

client_socket.send(client_public_key) # send the clients public key
